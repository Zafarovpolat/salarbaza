...
### 🤖 Telegram Bot — Flow взаимодействия

#### Пользовательский flow
```
/start (первый раз)
  ├── 🌿 Двуязычное приветствие
  ├── 🌐 Выбор языка: O'zbekcha / Русский
  │   ├── lang_uz → сохранить в БД → кнопка "Открыть магазин"
  │   └── lang_ru → сохранить в БД → кнопка "Открыть магазин"
  └── Кнопка магазина → WebApp с ?lang= параметром

/start (повторный)
  ├── Язык уже выбран → сразу приветствие + кнопка магазина
  └── Кнопка "Сменить язык" → повторный выбор

/start category_<slug>  → Deep Link → /catalog/<slug>
/start product_<slug>   → Deep Link → /product/<slug>
/start promo_<slug>     → Deep Link → /promotion/<slug>

/help → Справка + контакты + кнопка магазина
```

#### Admin flow (заказы)
```
Новый заказ → уведомление в админ-чат
  ├── ✅ confirm_<id> → CONFIRMED → уведомление пользователю
  ├── ❌ cancel_<id>  → CANCELLED → уведомление пользователю
  ├── 📦 ship_<id>    → SHIPPED   → уведомление пользователю
  ├── 🏠 deliver_<id> → DELIVERED → уведомление пользователю
  └── 📋 details_<id> → показать детали заказа
```

#### Компоненты бота
| Файл | Назначение |
|------|-----------|
| `commands.ts` | `/start` (deep link, выбор языка), `/help` |
| `handlers.ts` | Callback queries (язык, заказы, статусы) |
| `keyboards.ts` | Inline клавиатуры (язык, магазин, заказы) |
| `index.ts` | Инициализация бота, webhook/polling |

### 👤 Для пользователя (Telegram Bot)

| Функция | Статус |
|---------|--------|
| 🌿 Двуязычное приветствие (UZ/RU) | ✅ |
| 🌐 Выбор языка с сохранением в БД | ✅ |
| 🛍 Кнопка «Открыть магазин» (WebApp) | ✅ |
| 🔄 Повторный запуск — сразу магазин (если язык выбран) | ✅ |
| 🌐 Смена языка через inline-кнопку | ✅ |
| 🔗 Deep Link: `/start category_xxx` | ✅ |
| 🔗 Deep Link: `/start product_xxx` | ✅ |
| 🔗 Deep Link: `/start promo_xxx` | ✅ |
| ℹ️ Команда `/help` с контактами | ✅ |
...
| ❌ Отмена заказа | ✅ |
| 📦 Статус «Готов» (SHIPPED) | ✅ |
| 🏠 Статус «Доставлен» (DELIVERED) | ✅ |
| 📋 Детали заказа | ✅ |
