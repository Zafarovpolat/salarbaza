...
1. **База данных** - Настроить PostgreSQL на Supabase/Railway/Render
2. **Seed данных** - Выполнить `npm run db:seed` для загрузки товаров из JSON
3. **BOT_TOKEN** - Получить токен от @BotFather и настроить

### 🟡 Средний приоритет

1. **Изображения товаров** - Загрузить на CDN (Cloudinary/S3/Supabase Storage)
2. **Favorites API** - Синхронизация избранного с бэкендом
3. **Telegram Bot Webhook** - Настроить webhook для production

### 🟢 Низкий приоритет

1. **Redis кэширование** - Опционально для performance
2. **Push уведомления** - Уведомления о статусе заказа
3. **Admin Dashboard** - Веб-панель статистики
