...
def main():
    ...
    for p in products_with_suffix:
        ...
        if new_code.lower() in all_codes and new_code.lower() != old_code.lower():
            # Conflict — can't rename. But maybe the conflicting product IS this
            # one's root. Let's verify via Bito product name.
            ...
            if bito_p:
                ...
                if expected_code.lower() == new_code.lower():
                    # The root code is taken by a different product — skip
                    continue
            continue
        ...
        if bito_p:
            ...
            expected_code = title_case_root(normalize_root(root)) if root else ""
            if expected_code.lower() != new_code.lower():
                # The suffix isn't a dedup — it's part of the real code (e.g. "0278-140")
                continue

        code_updates.append((new_code, old_code, p["id"]))
        # Mark new_code as used, remove old
    ...
    # (Many products have nameRu = code, e.g. "C-6-2" -> should be "C-6")
    print("--- PHASE 3: Fix products.nameRu/nameUz matching old code ---")
    name_updates: List[Tuple[str, str, str]] = []  # (new_name, old_name, product_id)
    if not dry_run:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            for new_code, old_code, pid in code_updates:
                cur.execute(
    ...
    if dry_run:
        print("\n  ⚠️  No changes applied. Run with --apply to execute.")
    else:
